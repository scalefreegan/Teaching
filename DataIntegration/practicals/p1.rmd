...
> Run  k-means  (# clusters = 2) on the spiral data. You can load this data set with `data(spirals)`. K-means can be run with the commmand `kmeans`. Specify the number of clusters with `centers = 2`.
> 
...
> Run spectral clustering (# clusters = 2) on the `spirals` data. Spectral clustering is called with the function `specc`. Like k-means, specifiy the number of clusters with `centers = 2`. Once again, plot the data with clusters annotated. 
>
...
We will return again to spectral clustering with more theoretical and practical informaiton in the subsequent [lecture](http://scalefreegan.github.io/Teaching/DataIntegration/lectures/l2.html) and [tutorial](http://scalefreegan.github.io/Teaching/DataIntegration/practicals/p2.html)
...
Plotting the graph can take some time, so I've simply included an image of it below. 
...
<img src="http://scalefreegan.github.io/Teaching/DataIntegration/lectures/resources/labgraph.png" width="500px" />
