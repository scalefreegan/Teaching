...
>
> Compute node centrality. Print the 10 nodes with the highest modularity
>
...
```{r echo = FALSE}
library(knitr)
kable(as.data.frame(t(sort(cc, decreasing = T)[1:10])), format = "markdown")
```
...
For the remainder of the course, Cytoscape will be used to extend our analysis of the integrated graph kernel network and the communities derived from it by link-community detection.
